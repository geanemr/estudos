...
fetch(url, options)
.then(response => response.json()) // o then é só pra ver a postagem, porque o post funciona sem ele
.then(json => console.log('post:',json)); //{aula: 'JavaScript', id: 101}
...
.then(r => r.json())
.then(r => console.log('getPosts:', r)) //100) [{…}, {…}...
...
.then(r => r.json())
.then(r => console.log('newPost:', r)) //{titulo: 'JavaScript', id: 101}
...
.then(r => r.json())
.then(r => console.log('put:', r))  //{titulo: 'HTML', id: 1}
...
// Se o servidor não permitir o acesso, este será bloqueado. É possível passar por cima do bloqueio utilizando um proxy (não recomendado).
