...
function executarCallback(event) {
  const currentTarget = event.currentTarget; // todo item selecionado. Pode usar o 'this'.
  const target = event.target; // onde o clique ocorreu
  const type = event.type; // tipo de evento
  const path = event.path;
  console.log('executarCallbackEvents:' ,currentTarget, target, type, path);
}
...
linksInternos.forEach((link)=> link.addEventListener('click', handleLink))


// Selecione todos os elementos do site começando a partir do body,
// ao clique mostre exatamente quais elementos estão sendo clicados
const todosElementos = document.querySelectorAll('body *') //todos dentro de body
